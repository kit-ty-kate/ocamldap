  ...
  let errors = 
    Hashtbl.fold
	 ...
	 let check_error errors attr = 
	   ...
	   with 
	       Invalid_attribute _ -> 
		 (oc, Undefined_must_or_may attr) :: errors
	     | Non_unique_attribute_alias attr -> 
		 (oc, Non_unique_name_for_must_or_may attr) :: errors
	 in
	   ...
	   List.fold_left
		...
		with 
		    Invalid_objectclass _ -> 
		      (oc, Undefined_superior_oc sup) :: errors
		  | Non_unique_objectclass_alias _ ->
		      (oc, Non_unique_name_for_superior_oc sup) :: errors)
	     errors
	     sups)
      schema.objectclasses
      errors
  ...
  let errors = 
    Hashtbl.fold
	 ...
	 let attr = Lcstring.to_string attr in
	   ...
	   else (attr, Unknown_syntax syntax) :: errors)
      schema.attributes
      errors
  in
	 ...
	 let attrname = Lcstring.to_string attrname in
	   try
		 ...
		 Some mrule -> errors
		   ...
		   else (attrname, No_equality_matching_rule_defined) :: errors
	     end
	   with _ -> (attrname, No_equality_matching_rule_defined) :: errors)
      schema.attributes
      errors
  in
    errors
